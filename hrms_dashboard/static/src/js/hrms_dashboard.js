...
//             'click .hr_contract':'hr_contract',
             ...
             'click .hr_e_calendar': 'hr_e_calendar',
             'click .hr_notify_form': 'hr_notify_form',
//             'click .login_broad_factor': 'employee_broad_factor',
            "click .o_hr_attendance_sign_in_out_icon": function(ev) {
            this.$('.o_hr_attendance_sign_in_out_icon').attr("disabled", "disabled");
            this.update_attendance(ev);
        },
    ...
    async update_attendance(ev) {
        var self = this;
        if (self.login_employee.attendance_state == 'checked_in'){
            await this._rpc({
                model: 'hr.employee',
                method: 'update_attendance_today',
                args: [[self.login_employee.id]],
            })
        }
        this._rpc({
            model: 'hr.employee',
...
//        today = mm + '/' + dd + '/' + yyyy;
    ...
    hr_e_calendar:function(e) {
        ...
        this.do_action({
            ...
            domain:  ['|', ['employee_id.company_id', 'in', session.user_context.allowed_company_ids],'&', ['multi_employee', '=', true],'&', ['state', 'in', ['draft', 'confirm', 'validate1']],['employee_ids.company_id', 'in', session.user_context.allowed_company_ids]],
            // domain:  [ ['employee_id','=', this.login_employee.id] ],
            target: 'current'
        }, options)
    },
    ...
    hr_notify_form: async function(e) {
        var self = this;   
        e.stopPropagation();
        var recordID = await e.currentTarget.firstElementChild.getAttribute('value');
        e.preventDefault();
        this.do_action({
            name: _t("Notify Form"),
            type: 'ir.actions.act_window',
